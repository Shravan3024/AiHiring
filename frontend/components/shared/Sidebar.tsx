...
const mdNav = [
  { label: "Command Center", href: "/md", icon: LayoutDashboard },
  { label: "Applications", href: "/md/applications", icon: Users },
  { label: "Executive Decision", href: "/md/decision", icon: Shield },
  { label: "Analytics", href: "/md/analytics", icon: BarChart3 },
];
    ...
    >
      ...
      <div className="relative flex items-center justify-between px-5 h-20 border-b border-white/30 bg-white/20">
        <div className={cn("flex items-center gap-3 transition-all duration-500", !sidebarOpen && "scale-0 invisible w-0")}>
          ...
          <div className="flex flex-col">
            <span className="font-black text-xs tracking-[0.2em] uppercase mb-0.5 text-foreground">AI Hiring System</span>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {!sidebarOpen && (
                <div className="absolute left-[calc(100%+10px)] px-4 py-2 bg-card text-foreground text-[10px] font-black rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50 uppercase tracking-widest border border-border shadow-sm">
                  {item.label}
                </div>
              )}
            </Link>
      ...
      <div className="p-4 border-t border-white/30 space-y-3 bg-white/20">
        <button
          ...
          className={cn(
            "w-full flex items-center justify-between px-4 py-3.5 rounded-lg transition-all group border border-border/50 shadow-sm",
            theme === "dark"
        ...
        <button
          ...
          className="w-full flex items-center gap-4 px-4 py-4 rounded-lg text-destructive/60 hover:bg-destructive/10 hover:text-destructive transition-all group uppercase text-[10px] font-black tracking-widest"
        >
