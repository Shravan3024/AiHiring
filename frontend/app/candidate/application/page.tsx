          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card className="border-none shadow-sm rounded-[32px] bg-white p-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center"><TrendingUp className="w-6 h-6" /></div>
                <div>
            ...
            <Card className="border-none shadow-sm rounded-[32px] bg-white p-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center"><CheckCircle className="w-6 h-6" /></div>
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Recommended</p>
                  <p className="text-2xl font-bold text-slate-900">{applications.filter((a: any) => a.status === "RECOMMENDED_BY_AI").length}</p>
                </div>
              </div>
            </Card>
            <Card className="border-none shadow-sm rounded-[32px] bg-white p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">{applications.filter((a: any) => a.status === "PROCEED_TO_HR").length}</p>
                </div>
              </div>
            </Card>
            <Card className="border-none shadow-sm rounded-[32px] bg-white p-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-red-50 text-red-600 rounded-2xl flex items-center justify-center"><XCircle className="w-6 h-6" /></div>
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Rejected</p>
                  <p className="text-2xl font-bold text-slate-900">{applications.filter((a: any) => a.status === "AUTO_REJECTED").length}</p>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:max-w-md">
              ...
              <Input
                ...
                className="w-full pl-12 h-12 bg-white border-slate-100 rounded-2xl shadow-sm font-medium focus:ring-2 focus:ring-blue-100 transition-all"
              />
            </div>
            <Button variant="outline" className="h-12 px-6 rounded-2xl border-slate-100 bg-white font-bold gap-2 text-slate-600">
            ...
            <Table>
              <TableHeader className="bg-slate-50/50">
                <TableRow className="border-slate-50 hover:bg-transparent">
                  <TableHead className="px-8 py-6 font-bold text-slate-400 uppercase text-[10px] tracking-widest">Position & Company</TableHead>
                      ...
                      <TableCell className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400 font-medium">Mask Polymers Pvt. Ltd.</p>
                          </div>
                        </div>
                      </TableCell>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
           {apps.length > 0 ? apps.map((app: any, idx: number) => {
                ...
                <Card key={idx} className="border-none shadow-sm rounded-[32px] bg-white overflow-hidden p-8">
                   <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
                      <div className="flex items-center gap-6">
                         ...
                         <div>
                            <h3 className="text-2xl font-bold text-slate-900">{app.jobId?.title || "Role Title"}</h3>
             ...
             <Card className="border-none shadow-sm rounded-[32px] bg-white p-20 text-center">
                ...
                <Button className="mt-8 bg-blue-600 hover:bg-blue-700 h-12 px-10 rounded-xl" onClick={() => setActiveTab("jobs")}>Browse Jobs</Button>
             </Card>
           )}
        </div>
      )}
      ...
      {activeTab === "jobs" && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {jobs.map((job: any) => (
                <Card key={job._id} className="border-none shadow-sm rounded-[32px] bg-white p-10 group hover:shadow-xl hover:-translate-y-1 transition-all">
                   ...
                   <p className="text-slate-500 mt-6 leading-relaxed line-clamp-2">{job.description || "Join our team at Mask Polymers and help us build the future of AI-powered industrial solutions."}</p>
                   <Button 
                      className="w-full mt-10 h-14 rounded-2xl bg-slate-900 hover:bg-blue-600 text-white font-bold transition-all shadow-lg shadow-slate-200"
