         ...
         <div className="min-h-screen bg-white flex flex-col items-center py-12 px-6 overflow-y-auto">
            <div className="w-full max-w-4xl flex items-center justify-between mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h1 className="text-lg font-bold text-gray-900 tracking-tight">AI Hiring System <span className="text-blue-600">Recruitment</span></h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full max-w-6xl">
               <div className="lg:col-span-12 space-y-8">
                  <div className="bg-white border rounded-lg p-6 shadow-sm">
                     <h2 className="text-xl font-extrabold text-gray-900 mb-6 flex items-center gap-3">Assessment Overview</h2>
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        {[
                           ...
                           <div key={i} className="p-6 rounded-lg bg-gray-50 border border-gray-100 group hover:border-blue-200 transition-colors">
                  ...
                  <div className="bg-gray-900 rounded-lg p-6 flex flex-col md:flex-row items-center justify-between gap-5">
                     ...
                     <Button
                        ...
                        className={cn(
                           "h-12 px-16 rounded-[1.5rem] font-bold text-lg uppercase tracking-widest transition-all",
   ...
   if (!isFullScreen && !isSubmitting) return (
      <div className="min-h-screen bg-black flex items-center justify-center font-mono p-12 text-white">
         <div className="text-center">
            ...
            <Button onClick={handleFullScreen} className="mt-8 bg-white text-black font-black uppercase px-16 h-12 rounded-lg">Re-Enable Shield</Button>
         </div>
      </div>
   );
      ...
      <div ref={containerRef} className="min-h-screen bg-[#050505] text-slate-300 font-mono flex flex-col">
         <div className="h-28 border-b border-slate-900 bg-black/95 flex items-center justify-between px-16 sticky top-0 z-[100]">
            <div className="flex items-center gap-5">
               <div className="h-14 w-14 bg-white rounded-lg flex items-center justify-center text-black font-black text-lg">M</div>
               <h1 className="text-sm font-black text-white uppercase tracking-widest">Technical Subnet 01</h1>
            </div>
            <div className="flex items-center gap-12">
               <div className="flex items-center gap-4">
                  <Clock className="w-5 h-5 text-blue-500" />
                  <span className="text-xl font-black text-white tabular-nums">{timeLeft !== null ? formatTime(timeLeft) : "--:--"}</span>
               </div>
               <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-emerald-600 h-16 px-12 rounded-lg">FINISH TEST</Button>
            </div>
         </div>
         ...
         <div className="flex-1 flex gap-12 p-16">
            <div className="flex-1 flex flex-col gap-5">
               <Card className="flex-1 border-slate-900 bg-black rounded-[3.5rem] overflow-hidden flex flex-col border-2">
                  <CardHeader className="p-16 border-b border-slate-900 bg-slate-950/50">
                     <h2 className="text-lg font-black text-white leading-tight underline decoration-blue-500/30 underline-offset-8">
                        {currentQ.question}
                     </h2>
                  </CardHeader>
                  <CardContent className="p-16 space-y-6 overflow-y-auto">
                     {currentQ.options && currentQ.options.length > 0 ? (
                        currentQ.options.map((opt, i) => (
                           <button key={i} onClick={() => handleAnswer(opt)} className={cn("w-full p-5 text-left rounded-lg border-2 transition-all flex items-center gap-5", answers[currentQ.id] === opt ? "bg-blue-600 border-blue-500" : "bg-black border-slate-900")}>
                              <div className="w-12 h-12 rounded-lg flex items-center justify-center font-black border-2">{String.fromCharCode(65 + i)}</div>
                              <span className="text-xl font-bold">{opt}</span>
                           </button>
                        ))
                     ) : (
                        <textarea value={answers[currentQ.id] || ""} onChange={(e) => handleAnswer(e.target.value)} className="w-full h-96 bg-black border-2 border-slate-900 rounded-lg p-12 text-lg" />
                     )}
                  </CardContent>
                  <CardFooter className="p-16 flex justify-between bg-black">
                     <Button variant="outline" disabled={currentQuestionIndex === 0} onClick={() => setCurrentQuestionIndex(v => v - 1)} className="h-12 px-12 rounded-lg">BACK</Button>
                     <Button disabled={currentQuestionIndex === assessment.questions.length - 1} onClick={() => setCurrentQuestionIndex(v => v + 1)} className="bg-white text-black h-12 px-16 rounded-lg">NEXT</Button>
                  </CardFooter>
               </Card>
            </div>
            ...
            <div className="w-[450px] flex flex-col gap-6">
               <div className="aspect-[3/4] bg-slate-950 rounded-[4rem] border-2 border-slate-900 overflow-hidden relative grayscale contrast-125">
                  ...
                  <div className="absolute top-6 left-10 p-3 bg-red-600 rounded-full animate-pulse" />
               </div>
               <Card className="flex-1 border-slate-900 bg-slate-950/40 rounded-[4rem] p-12 border-2 text-center">
                  ...
                  <div className="grid grid-cols-5 gap-4">
                     {assessment.questions.map((q, i) => (
                        <button key={q.id} onClick={() => setCurrentQuestionIndex(i)} className={cn("aspect-square rounded-lg text-xs font-black border-2", i === currentQuestionIndex ? "bg-white text-black" : answers[q.id] ? "bg-blue-600/20 border-blue-500" : "bg-black border-slate-900")}>{i + 1}</button>
                     ))}
                  </div>
               </Card>
            </div>
