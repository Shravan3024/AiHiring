      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className={cn("p-6 rounded-[2rem] bg-white/5 border backdrop-blur-md flex items-center justify-between group hover:bg-white/[0.08] transition-all", stat.border)}>
              <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">{stat.label}</p>
                <p className="text-4xl font-bold text-white tabular-nums tracking-tighter">{stat.val}</p>
              </div>
        ...
        {bottleneckStage && (
          <div className="p-4 rounded-2xl bg-gradient-to-r from-orange-500/10 to-transparent border border-orange-500/20 flex items-center gap-4 animate-in fade-in duration-1000">
             ...
             <button className="text-[10px] font-black uppercase text-white/40 hover:text-white transition-colors tracking-widest">Accelerate Workflow</button>
          </div>
        )}
              ...
              <div className="bg-white/5 border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-xl">
                 <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                       ...
                       <h3 className="text-lg font-bold text-white tracking-tight">Candidates Awaiting Decision</h3>
                    </div>
                    <div className="flex items-center gap-4">
                       <div className="relative group">
                          ...
                          <input 
                            ...
                            className="bg-black/40 border border-white/10 rounded-full py-2 pl-10 pr-4 text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500/50 w-64 transition-all"
                          />
                       </div>
                 ...
                 <div className="p-4 min-h-[400px]">
                    {isLoading ? (
                      <div className="flex items-center justify-center h-64 flex-col gap-4">
                         ...
                         <p className="text-xs font-black uppercase tracking-widest text-white/20">Syncing Pipeline...</p>
                      </div>
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                         ...
                         <div className="space-y-1">
                            <p className="text-white font-bold opacity-40">Queue cleared</p>
                            <p className="text-xs text-white/20 uppercase tracking-widest">No candidates awaiting admin approval</p>
                         </div>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 gap-3">
                        {pendingApprovals.map((app: any, idx) => (
                          <div 
                            ...
                            className="group flex flex-wrap items-center justify-between p-5 bg-white/[0.01] hover:bg-white/[0.04] border border-white/5 hover:border-white/10 rounded-2xl transition-all duration-300 relative overflow-hidden"
                          >
                            {/* Decorative line */}
                            ...
                            <div className="flex items-center gap-5 min-w-[280px]">
                               <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center shrink-0 border border-white/10">
                            ...
                            <div className="flex items-center gap-8">
                               <div className="flex flex-col items-end gap-1">
                                  <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">AI Fit Band</span>
                                  <Badge className={cn(
                                    "px-3 py-1 rounded-full uppercase text-[9px] font-black tracking-widest border border-white/10",
                               ...
                               <div className="flex flex-col items-end gap-1">
                                   ...
                                   <span className="text-xs text-white font-medium opacity-60">
                                      {app.applicationStatus?.replace(/_/g, " ") || "Pending Decision"}
                                   </span>
                               </div>

                               <button 
                                 className="ml-4 w-10 h-10 rounded-full bg-blue-600 hover:bg-blue-500 flex items-center justify-center text-white transition-all shadow-lg shadow-blue-900/40 hover:scale-110 active:scale-95"
                                 onClick={() => window.location.href = `/admin/candidate/${app.applicationId}`}
                               >
           ...
           <div className="space-y-6">
              <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-6 backdrop-blur-xl">
                 <div className="space-y-2">
                    ...
                    <h4 className="text-white font-bold tracking-tight">Active Funnel</h4>
                    <p className="text-xs text-white/40 leading-relaxed font-medium">Real-time distribution of candidates across core hiring stages.</p>
                 </div>
                    ...
                    {bottleneck.length > 0 ? bottleneck.map((s, idx) => (
                      <div key={idx} className="space-y-2">
                         <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                            <span className="text-white/40">{s.stage?.replace(/_/g, " ")}</span>
                      ...
                      <p className="text-[10px] uppercase font-black text-white/10 tracking-widest text-center py-10 italic">Awaiting Metrics...</p>
                    )}
                 </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                 ...
                 <div className="relative z-10 space-y-4">
                    ...
                    <p className="text-xs font-medium text-white/70 leading-relaxed">Automated status updates are currently enforcing <span className="font-black border-b border-white/30">Approval Flow v2.4</span>. Ensure all compliance flags are cleared before final selection.</p>
