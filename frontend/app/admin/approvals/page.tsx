...
export default function AdminApprovalsPage() {
  ...
  const { data: funnelData } = useQuery({
    ...
    queryFn: () => adminApi.getFunnel().then(r => r.data?.data || r.data || []),
  ...
  const funnel: any[] = Array.isArray(funnelData) ? funnelData : [];
  ...
  const bottleneckInfo = !Array.isArray(bottleneckData) ? bottleneckData : null;
          ...
          <div className="p-4 rounded-lg bg-gradient-to-r from-orange-500/10 to-transparent border border-orange-500/20 flex items-center gap-4 animate-in fade-in duration-1000">
             ...
             <div className="flex-1">
               <p className="text-sm font-medium text-slate-900/80">
                 Efficiency Alert: <span className="text-orange-400 font-bold">{bottleneckInfo.pending_approvals} candidates</span> are currently awaiting final HR Review.
               </p>
             </div>
                 ...
                 <div className="p-4 min-h-[400px]">
                    {isLoading ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                         ...
                         <div className="space-y-1">
                            <p className="text-slate-900 font-bold opacity-40">No matches found</p>
                            <p className="text-xs text-slate-400 uppercase tracking-widest">Try adjusting your filter terms</p>
                         </div>
                      </div>
                    ) : (
                        ...
                        {filteredApprovals.map((app: any, idx) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-5">
                               ...
                               <div className="space-y-1">
                                  <p className="text-slate-900 font-bold text-sm group-hover:text-blue-400 transition-colors truncate max-w-[200px]">
                                    {app.candidateName || `Candidate #${app.applicationId}`}
                                  </p>
                                  <div className="flex items-center gap-2 text-[10px] uppercase font-black tracking-widest text-slate-500 truncate max-w-[200px]">
                            ...
                            <div className="flex flex-col items-center gap-1">
                               ...
                               <Badge className={cn(
                                 "px-3 py-1 rounded-full uppercase text-[9px] font-black tracking-widest border border-slate-200 whitespace-nowrap",
                            ...
                            <div className="flex flex-col items-center md:items-end gap-1">
                                ...
                                <span className="text-xs text-slate-900 font-medium opacity-60 text-right">
                                   {app.applicationStatus?.replace(/_/g, " ") || "Pending Decision"}
                                </span>
                            </div>

                            <button 
                              className="w-10 h-10 rounded-full bg-blue-600 hover:bg-blue-500 flex items-center justify-center text-white transition-all shadow-lg shadow-blue-500/20 hover:scale-110 active:scale-95 ml-auto"
           ...
           <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-lg p-8 text-slate-900 relative overflow-hidden group">
                 ...
                 <div className="relative z-10 space-y-4">
                    <h4 className="font-bold tracking-tight text-xl leading-tight text-white">Admin System Governance</h4>
                    <p className="text-xs font-medium text-blue-100 leading-relaxed">Automated status updates are currently enforcing <span className="font-black border-b border-blue-300">Approval Flow v2.4</span>. Ensure all compliance flags are cleared before final selection.</p>
                    <button 
                      onClick={() => window.location.href = '/admin/hr-management'}
                      className="pt-2 text-[10px] font-black uppercase tracking-widest border-b border-white flex items-center gap-2 group-hover:gap-4 transition-all text-white"
                    >
                      Audit Policies
                    </button>
                 </div>
